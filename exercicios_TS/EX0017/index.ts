...
// Implemente um sistema de envio de notificação onde o envio de E-mails ou SMS depende de interfaces abstraídas

// Crie dois exemplo: um com interfaces e outro com classes abstratas
// Dependência de alto nível não deve depender de dependências de baixo nível. Ambas devem depender de abstrações.

// Classe de alto nível - abstração, pois vai conter classes de baixo níveis
...
class NotificacaoEmail implements EnvioNotificacao{
    enviarNotificacao(notificacaoPorEmail: string, mensagem: string): void {
        console.log(`Email: ${notificacaoPorEmail} \n Mensagem: ${mensagem}`);
    }
}
// Classes de baixo nível operacional que envia SMS - implementação concreta
class NotificacaoSMS implements EnvioNotificacao{
    enviarNotificacao(notificacaoPorSMS: string, mensagem: string): void {
