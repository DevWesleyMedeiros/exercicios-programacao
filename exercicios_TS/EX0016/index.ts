...
class Empresa implements PessoaJuridica {
  ...
  // se implemento uma classe, se os modificadores de acesso forem publicos, não preciso declarar eles, mas se forem privados ou protegidos, preciso declarar eles, também na interface, para que a classe possa implementar a interface, caso contrário, o TypeScript vai reclamar que a classe não implementa a interface corretamente
...
class CalcularIdade {
  static calcularIdade(pessoa: PessoaNatural): number {
    ...
    if (
      ...
      (dataAtual.getMonth() === nascimento.getMonth() && dataAtual.getDate() < nascimento.getDate())
    ) {
      idade--
    }
    return idade
...
class GerarNumeroTitulo {
  static numeroTitulo(): string {
    const bloco1 = Math.floor(Math.random() * 10000)
      .toString()
      .padStart(4, '0')
    const bloco2 = Math.floor(Math.random() * 10000)
      .toString()
      .padStart(4, '0')
    const bloco3 = Math.floor(Math.random() * 10000)
      .toString()
      .padStart(4, '0')
...
function validarCPF(cpf: string): boolean {
  ...
  const calcDigito = (base: string, pesos: number[]) =>
    pesos.reduce((soma, peso, i) => soma + parseInt(base[i]) * peso, 0) % 11 < 2
      ? 0
